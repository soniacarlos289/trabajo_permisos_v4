...
if (request.getParameter("MM_update") != null &&
  ...
  String MM_fieldsStr = "ID_JS|value|ID_DELEGADO_JS|value|ID_JA|value";
  String MM_columnsStr = "id_ver_plani_1|none,none,NULL|id_ver_plani_2|none,none,NULL|id_ver_plani_3|none,none,NULL";
...
PreparedStatement StatementRSQUERYJS = ConnRSQUERYJS.prepareStatement("SELECT f.id_funcionario,lpad(id_ver_plani_1,6,' ') as id_js  ,lpad(pe.nombre || ' ' || pe.ape1,60,' ') as nombre  FROM personal_new pe, funcionario_firma f  WHERE f.id_funcionario='" + RSQUERYJS__MMColParam1 + "' and         pe.id_funcionario=id_ver_plani_1");
...
PreparedStatement StatementRSQUERYJA = ConnRSQUERYJA.prepareStatement("SELECT f.id_funcionario,lpad(id_ver_plani_3,6,' ') as id_ja  ,lpad(pe.nombre || ' ' || pe.ape1,60,' ') as nombre  FROM personal_new pe, funcionario_firma f  WHERE f.id_funcionario='" + RSQUERYJA__MMColParam1 + "' and         pe.id_funcionario=id_ver_plani_3");
...
PreparedStatement StatementRSTODOS = ConnRSTODOS.prepareStatement("SELECT distinct id_f, nombre, ape  from (  SELECT  '0' as id_f, ' ' as nombre ,'AAAAA' as ape from dual UNION   SELECT lpad(id_funcionario, 6, ' ') as id_f,rpad(' ' || pe.ape1 || ' ' || pe.ape2 ||' ,'|| pe.nombre , 60, ' ') as nombre,ape1 || ape2 as ape FROM personal_new pe  where fecha_baja is null or fecha_baja > sysdate -60  ) ORDER BY ape");
...
PreparedStatement StatementRSDELEGADOJS = ConnRSDELEGADOJS.prepareStatement("SELECT f.id_funcionario,lpad(id_ver_plani_2,6,' ') as id_delegado_js  ,lpad(pe.nombre || ' ' || pe.ape1,60,' ') as nombre  FROM personal_new pe, funcionario_firma f  WHERE f.id_funcionario='" + RSDELEGADOJS__MMColParam1 + "' and         pe.id_funcionario=id_ver_plani_2");
...
%>
    ...
    <tr>
      <th colspan="3" scope="row"><div align="center">
          <input type="submit" name="Submit2" value="Guardar Permisos sobre Planificador">
      </div></th>
    </tr>
  </table>
