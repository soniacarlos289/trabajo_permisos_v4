...
if (request.getParameter("MM_update") != null &&
  ...
  MM_editColumn = "ID_FUNCIONARIO";
  MM_recordId   = "" + request.getParameter("MM_recordId") + "";
  MM_editRedirectUrl = "index.jsp";
  String MM_fieldsStr = "ID_JS|value|ID_DELEGADO_JS|value|ID_JA|value|ID_DELEGADO_FIRMA|value";
  String MM_columnsStr = "ID_JS|none,none,NULL|ID_DELEGADO_JS|none,none,NULL|ID_JA|none,none,NULL|ID_DELEGADO_FIRMA|none,none,NULL";
...
PreparedStatement StatementRSQUERYJS = ConnRSQUERYJS.prepareStatement("SELECT f.id_funcionario,lpad(id_js,6,'0') as id_js  ,lpad(pe.nombre || ' ' || pe.ape1||' ' || pe.ape2,60,' ') as nombre  FROM personal_new pe, funcionario_firma f  WHERE f.id_funcionario='" + RSQUERYJS__MMColParam1 + "' and         pe.id_funcionario=id_js");
...
PreparedStatement StatementRSQUERYJA = ConnRSQUERYJA.prepareStatement("SELECT f.id_funcionario,lpad(id_ja,6,'0') as id_ja  ,lpad(pe.nombre || ' ' || pe.ape1||' ' || pe.ape2,60,' ') as nombre  FROM personal_new pe, funcionario_firma f  WHERE f.id_funcionario='" + RSQUERYJA__MMColParam1 + "' and         pe.id_funcionario=id_ja");
...
PreparedStatement StatementRSTODOS = ConnRSTODOS.prepareStatement("SELECT distinct id_f, nombre, ape  from (     SELECT lpad(id_funcionario, 6, '0') as id_f,rpad( ' ' || pe.ape1 || ' ' || pe.ape2 ||' ,'|| pe.nombre , 60, ' ') as nombre,ape1 || ape2 as ape FROM personal_new pe  where fecha_baja is  null or fecha_baja > sysdate -60  ) ORDER BY ape");
...
PreparedStatement StatementRSDELEGADOJS = ConnRSDELEGADOJS.prepareStatement("SELECT f.id_funcionario,lpad(id_delegado_js,6,'0') as id_delegado_js  ,lpad(pe.nombre || ' ' || pe.ape1||' ' || pe.ape2,60,' ') as nombre,to_char(ID_DELEGADO_FIRMA) as ID_DELEGADO_FIRMA  FROM personal_new pe, funcionario_firma f  WHERE f.id_funcionario='" + RSDELEGADOJS__MMColParam1 + "' and         pe.id_funcionario=id_delegado_js");
...
<tr><td><form name="form1" method="POST" action="<%=MM_editAction%>">
  <table width="100%"  border="0" cellspacing="2" cellpadding="4">
    <tr>
      <td colspan="3" bgcolor="#FFCCCC" scope="col"><div align="center">Personas que tienen que <strong>AUTORIZAR y VER</strong> los permisos y ausencias de este funcionario. </div></td>
    </tr>
    <tr>
      ...
      <select name="ID_JS">
          <%
while (RSTODOS_hasData) {
%>
          <% if (!RSQUERYJS_isEmpty ) { %>
          <option value="<%=((RSTODOS.getObject("ID_F")!=null)?RSTODOS.getObject("ID_F"):"")%>" <%=(((RSTODOS.getObject("ID_F")).toString().equals(((((RSQUERYJS_data = RSQUERYJS.getObject("ID_JS"))==null || RSQUERYJS.wasNull())?"":RSQUERYJS_data)).toString()))?"selected=\"selected\"":"")%> ><%=((RSTODOS.getObject("NOMBRE")!=null)?RSTODOS.getObject("NOMBRE"):"")%></option>
          <% } else { %>
          <option value="<%=((RSTODOS.getObject("ID_F")!=null)?RSTODOS.getObject("ID_F"):"")%>"><%=((RSTODOS.getObject("NOMBRE")!=null)?RSTODOS.getObject("NOMBRE"):"")%></option>
          <% } /* end !RSQUERYJS_isEmpty */ %>
          <%
  RSTODOS_hasData = RSTODOS.next();
}
RSTODOS.close();
RSTODOS = StatementRSTODOS.executeQuery();
RSTODOS_hasData = RSTODOS.next();
RSTODOS_isEmpty = !RSTODOS_hasData;
%>
        </select>
      </label><input type="text" ID="ID_JS_CAM" size="25"><input type="button" name="" value="Buscar Funcionario"
      onClick="javascript:window.open('../Busqueda/index_busqueda_funcionario.jsp?CAMPO=ID_JS_CAM' ,null,'height=550,width=600,resizable=n0,scrollbars=no,status=no,toolbar=no ,menubar=no');"
      >
	 ...
	 <tr>
      ...
      <td colspan="2" bgcolor="#FFFFFF"><label>
        ...
        <select name="ID_DELEGADO_JS">
          <%
while (RSTODOS_hasData) {
%>
   <% if (!RSDELEGADOJS_isEmpty ) { %>
<option value="<%=((RSTODOS.getObject("ID_F")!=null)?RSTODOS.getObject("ID_F"):"")%>" <%=(((RSTODOS.getObject("ID_F")).toString().equals(((((RSDELEGADOJS_data = RSDELEGADOJS.getObject("ID_DELEGADO_JS"))==null || RSDELEGADOJS.wasNull())?"":RSDELEGADOJS_data)).toString()))?"selected=\"selected\"":"")%> ><%=((RSTODOS.getObject("NOMBRE")!=null)?RSTODOS.getObject("NOMBRE"):"")%></option>
    <% } else { %>
	 <option value="<%=((RSTODOS.getObject("ID_F")!=null)?RSTODOS.getObject("ID_F"):"")%>"><%=((RSTODOS.getObject("NOMBRE")!=null)?RSTODOS.getObject("NOMBRE"):"")%></option>
...
<option value="<%=((RSTODOS.getObject("ID_F")!=null)?RSTODOS.getObject("ID_F"):"")%>" <%=(((RSTODOS.getObject("ID_F")).toString().equals(((((RSQUERYJA_data = RSQUERYJA.getObject("ID_JA"))==null || RSQUERYJA.wasNull())?"":RSQUERYJA_data)).toString()))?"selected=\"selected\"":"")%> ><%=((RSTODOS.getObject("NOMBRE")!=null)?RSTODOS.getObject("NOMBRE"):"")%></option>
 <% } else { %>
 
 <option value="<%=((RSTODOS.getObject("ID_F")!=null)?RSTODOS.getObject("ID_F"):"")%>"><%=((RSTODOS.getObject("NOMBRE")!=null)?RSTODOS.getObject("NOMBRE"):"")%></option>
<% } /* end !RSQUERYJA_isEmpty */ %>
              <%
  RSTODOS_hasData = RSTODOS.next();
}
RSTODOS.close();
...
%>
 ...
 <tr>
  ...
  <input type="hidden" name="MM_recordId" value="<%=(((RSFUNCIONARIO_data = RSFUNCIONARIO.getObject("ID_FUNCIONARIO"))==null || RSFUNCIONARIO.wasNull())?"":RSFUNCIONARIO_data)%>">
</form>
    </td></tr>
                      </table>
