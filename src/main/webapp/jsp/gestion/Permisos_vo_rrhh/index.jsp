...
PreparedStatement StatementRSPENDIENTE = ConnRSPENDIENTE.prepareStatement("SELECT distinct DECODE(pe.JUSTIFICACION,'--','',CHEQUEA_ENLACE_FICHERO_JUS(pe.ID_ANO,pe.ID_FUNCIONARIO,ID_PERMISO,pe.id_estado,'P',2)) as JUSTI_FICHERO,pe.id_Funcionario,    rownum as FILA,            pe.ID_ANO,    pe.id_tipo_permiso as id_tipo_per,            ID_PERMISO,   ape1,ape2,                                                            initcap(Nombre) || ' ' || INITCAP(Ape1) || ' ' ||                       INITCAP(Ape2) as NOMBRES, to_char(pe.fecha_inicio, 'dd/mm/yyyy') ||  ' '  || decode(pe.id_tipo_permiso,15000, hora_inicio)  as FECHA_INICIO,pe.FECHA_INICIO as FECHA_INI, pe.FECHA_FIN as FECHA_FINI, to_char(pe.fecha_fin, 'dd/mm/yyyy')  ||  ' '  || decode(pe.id_tipo_permiso,15000, hora_fin)  as FECHA_FIN,     pe.num_dias || '-' || tipo_dias as NUM_DIAS, DEsc_tipo_permiso,pe.JUSTIFICACION as JUSTI  FROM   permiso pe,       (select distinct id_funcionario, NOMBRE, ape1, ape2 FROM personal_new) p,       tr_tipo_permiso tr WHERE ((id_estado in ('22') and pe.id_funcionario = p.id_funcionario))   and tr.id_tipo_permiso = pe.id_tipo_permiso   and tr.id_ano = pe.id_ano   and (ANULADO IS NULL OR ANULADO = 'NO')  ORDER BY " + PRCAMPO_ORDENA+ "");
...
<table width="85%" border="0" cellspacing="1" cellpadding="2">
  <form name="Sindicales" method="POST" action="firmar_select.jsp">
                                     <tr bgcolor="#CCFFCC">
                                         <td colspan="10" align="center"><b>Permisos pendientes de VºBº</b> <input type="hidden" name="TODO_T" ID="TODO_T" size="122" >
                                        <input type="hidden" name="ID_AUTORIZA" ID="ID_AUTORIZA" size="1" value="" >
                                        <input type="hidden" name="TIPO_T" ID="TIPO_T" size="1" value="P" >
                                      ...
                                      <tr bgcolor="#CCCCCC" >
                                        ...
                                        <td colspan="7" align="left"><input type="submit" value="Denegar selecionados" ID="DENI" name="DENI"  onClick="javascript:envia_unavez('0');">                                       
                                         &nbsp;Motivo denegación: &nbsp;<input type="text" name="ID_MOTIVO_DENEGA" ID="ID_MOTIVO_DENEGA" size="55" value="" >
                              ...
                              <tr> 
                                        ...
                                        <td width="5%" bgcolor="#CCCCCC" align="center"><span class="va10b">Num dias</span></td>
