...
{  
  response.setContentType("application/vnd.ms-excel");
	 if (!RSINFORME_isEmpty ) {
        response.setHeader("Content-Disposition", "attachment; filename=" + "Permisos_Ausencias_sin_Justificar_"+  (((RSINFORME_data = RSINFORME.getObject("FILTRO_1_TXT"))==null || RSINFORME.wasNull())?"":RSINFORME_data) + "_"  +  (((RSINFORME_data = RSINFORME.getObject("FILTRO_2_TXT"))==null || RSINFORME.wasNull())?"":RSINFORME_data) +".xls");
      } else { 
           response.setHeader("Content-Disposition", "attachment; filename=" + "Permisos_Ausencias_sin_Justificar"+".xls");
      }
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
 <tr>
   <th colspan="14" align=center bgcolor="#E6E6E6">Permisos/Ausencias en estas fechas</th>                                             
  ...
  <td colspan=14>
	...
	<% if (!RSDATOS_isEmpty ) { %>
	   <% while ((RSDATOS_hasData)&&(Repeat1__numRows-- != 0)) { 
               	 ...
               	 RSDIA= (String) (((RSDATOS_data = RSDATOS.getObject("CAMPO1"))==null || RSDATOS.wasNull())?"":RSDATOS_data); 
            ...
            <tr>
   	        	...
   	        	<td  colspan="2" align=center  > <%=(((RSDATOS_data = RSDATOS.getObject("CAMPO1"))==null || RSDATOS.wasNull())?"":RSDATOS_data)%></td>
   				<td  <%=RSCOLOR %>  align=center  ><%=(((RSDATOS_data = RSDATOS.getObject("CAMPO2"))==null || RSDATOS.wasNull())?"":RSDATOS_data)%></td>				
   				<td  <%=RSCOLOR %>  align=center  ><%=(((RSDATOS_data = RSDATOS.getObject("CAMPO3"))==null || RSDATOS.wasNull())?"":RSDATOS_data)%></td>
