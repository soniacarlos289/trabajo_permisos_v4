...
PreparedStatement StatementRSCABECERA = ConnRSCABECERA.prepareStatement("select to_char(sysdate,'dd/mm/yyyy hh24:mi') as FECHA_INFORME, DECODE(id_tipo_informe,2,1,id_tipo_informe) as id_tipo_informe, campo1, campo2, campo3, campo4, campo5, campo6, campo7, campo8, campo9, campo10, campo11, campo12, campo13, campo14, campo15, campo16, campo17, campo18, campo19, campo20 from fichaje_informe_cabecera where id_tipo_informe= 1");
...
PreparedStatement StatementRSDATOS = ConnRSDATOS.prepareStatement("select id_col,to_char(mod(id_col, 2)) as impar, campo1, campo2,campo3, campo31, campo4, campo5, campo6, campo7, campo8, campo9, campo10, campo11, campo12, campo13, campo14, campo15,  campo16, campo17, campo18, campo19, campo20 from fichaje_informe_campo where ID_EJECUCION="+ RS_INFORME +" ORDER BY  id_col");
...
{  
  response.setContentType("application/vnd.ms-excel");
	 if (!RSINFORME_isEmpty ) {
        response.setHeader("Content-Disposition", "attachment; filename=" + "Informe_Saldo_"+  (((RSINFORME_data = RSINFORME.getObject("FILTRO_1_TXT"))==null || RSINFORME.wasNull())?"":RSINFORME_data) + "_"  +  (((RSINFORME_data = RSINFORME.getObject("FILTRO_2_TXT"))==null || RSINFORME.wasNull())?"":RSINFORME_data) +".xls");
      } else { 
           response.setHeader("Content-Disposition", "attachment; filename=" + "Informe_Saldo_"+".xls");
      }
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
 <tr>
   <th colspan="14" align=center bgcolor="#E6E6E6"> Informe Saldo</th>                                             
 ...
 <tr>
  <td colspan=14>
    ...
    <table width="100%" border="1" cellspacing="1" cellpadding="1">
      <tr>
        <th colspan="2" align=center bgcolor="#F2F2F2" ><%=(((RSCABECERA_data = RSCABECERA.getObject("CAMPO1"))==null || RSCABECERA.wasNull())?"":RSCABECERA_data)%></th>
         <th  align=center bgcolor="#F2F2F2" ><%=(((RSCABECERA_data = RSCABECERA.getObject("CAMPO3"))==null || RSCABECERA.wasNull())?"":RSCABECERA_data)%>_PE</th>
        ...
        <th  align=center bgcolor="#F2F2F2" ><%=(((RSCABECERA_data = RSCABECERA.getObject("CAMPO7"))==null || RSCABECERA.wasNull())?"":RSCABECERA_data)%></th>
         <th  align=center bgcolor="#F2F2F2" ><%=(((RSCABECERA_data = RSCABECERA.getObject("CAMPO17"))==null || RSCABECERA.wasNull())?"":RSCABECERA_data)%></th>
        ...
        <th  align=center bgcolor="#F2F2F2" ><%=(((RSCABECERA_data = RSCABECERA.getObject("CAMPO3"))==null || RSCABECERA.wasNull())?"":RSCABECERA_data)%>_DIA</th>
        <th  align=center bgcolor="#F2F2F2" ><%=(((RSCABECERA_data = RSCABECERA.getObject("CAMPO16"))==null || RSCABECERA.wasNull())?"":RSCABECERA_data)%></th>
      
        <th  colspan="4" align=center bgcolor="#F2F2F2" ><%=(((RSCABECERA_data = RSCABECERA.getObject("CAMPO15"))==null || RSCABECERA.wasNull())?"":RSCABECERA_data)%></th>
     </tr>
	<% if (!RSDATOS_isEmpty ) { %>
	   <% while ((RSDATOS_hasData)&&(Repeat1__numRows-- != 0)) { 
              	   ...
              	   RSDIA= (String) (((RSDATOS_data = RSDATOS.getObject("CAMPO4"))==null || RSDATOS.wasNull())?"":RSDATOS_data); 
            ...
            <tr>
   	            ...
   	            <%  if (!RSFuncionario.equals(RSFuncionario_old ) ) { %>	
   	        	
   	        	<%=(((RSDATOS_data = RSDATOS.getObject("CAMPO1"))==null || RSDATOS.wasNull())?"":RSDATOS_data)%>-
   								<%=(((RSDATOS_data = RSDATOS.getObject("CAMPO2"))==null || RSDATOS.wasNull())?"":RSDATOS_data)%><%   } %></td>
   				<td  align=center  >
   				 <%  if (!RSFuncionario.equals(RSFuncionario_old ) ) { %>				
   			       <%=(((RSDATOS_data = RSDATOS.getObject("CAMPO31"))==null || RSDATOS.wasNull())?"":RSDATOS_data)%>
   			    <%   } %></td>  					
   				<td  <%=RSCOLOR %>  align=center  ><%=(((RSDATOS_data = RSDATOS.getObject("CAMPO4"))==null || RSDATOS.wasNull())?"":RSDATOS_data)%></td>
   				<td  <%=RSCOLOR %>  align=center  ><%=(((RSDATOS_data = RSDATOS.getObject("CAMPO5"))==null || RSDATOS.wasNull())?"":RSDATOS_data)%></td>
   				<td  <%=RSCOLOR %> align=center  ><%=(((RSDATOS_data = RSDATOS.getObject("CAMPO6"))==null || RSDATOS.wasNull())?"":RSDATOS_data)%></td>
   				<td  <%=RSCOLOR %> align=center  ><%=(((RSDATOS_data = RSDATOS.getObject("CAMPO7"))==null || RSDATOS.wasNull())?"":RSDATOS_data)%></td>
   				<td  <%=RSCOLOR %> align=center  ><%=(((RSDATOS_data = RSDATOS.getObject("CAMPO17"))==null || RSDATOS.wasNull())?"":RSDATOS_data)%></td>
   				
   				<td  <%=RSCOLOR %> align=center  >
   				    <%  if ( (!RSDIA.equals(RSDIA_old )) ||  (!RSFuncionario.equals(RSFuncionario_old ))) { %>	
   				 <%=(((RSDATOS_data = RSDATOS.getObject("CAMPO9"))==null || RSDATOS.wasNull())?"":RSDATOS_data)%><%   } %></td>
   				<td  <%=RSCOLOR %> align=center  > 	<%=(((RSDATOS_data = RSDATOS.getObject("CAMPO8"))==null || RSDATOS.wasNull())?"":RSDATOS_data)%></td>   				
   			    <td  <%=RSCOLOR %> align=center  >  <%  if ( (!RSDIA.equals(RSDIA_old )) ||  (!RSFuncionario.equals(RSFuncionario_old ))) { %>	<%=(((RSDATOS_data = RSDATOS.getObject("CAMPO10"))==null || RSDATOS.wasNull())?"":RSDATOS_data)%><%   } %></td>
   			
   				<td  <%=RSCOLOR %> align=center  > <%=(((RSDATOS_data = RSDATOS.getObject("CAMPO11"))==null || RSDATOS.wasNull())?"":RSDATOS_data)%></td>
   				<td  <%=RSCOLOR %> align=center  ><%=(((RSDATOS_data = RSDATOS.getObject("CAMPO16"))==null || RSDATOS.wasNull())?"":RSDATOS_data)%></td>   				
   				<td  <%=RSCOLOR %> colspan="4" align=center  ><%=(((RSDATOS_data = RSDATOS.getObject("CAMPO15"))==null || RSDATOS.wasNull())?"":RSDATOS_data)%></td>  
			</tr>
                              <%
  Repeat1__index++;
