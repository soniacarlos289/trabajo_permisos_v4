...
CallableStatement PERMI = ConnPERMI.prepareCall("{ call PERMISOS_ALTA_RRHH(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
...
PERMI.setString(14,PERMI__V_UNICO);
PERMI.setString(15,PERMI__V_IP);
PERMI.setString(16,PERMI__V_ID_USUARIO);
PERMI.registerOutParameter(17,Types.LONGVARCHAR);
PERMI.registerOutParameter(18,Types.LONGVARCHAR);
Object PERMI_data;
PERMI.execute();
%>
<%
...
PreparedStatement StatementRSTIPO_PERMISO = ConnRSTIPO_PERMISO.prepareStatement("SELECT DESC_TIPO_PERMISO  FROM tr_tipo_permiso  WHERE ID_ANO=" + RSTIPO_PERMISO__MMColParam1 + " and ID_TIPO_PERMISO=" + RSTIPO_PERMISO__MMColParam2 + "");
                                ...
                                <table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" bordercolor="#FFFFFF">
                                  <tr bgcolor="#f2f2f2"> 
                                    <td colspan="13" align="right" bgcolor="#E0E0E0"><div align="left"> </div>                                      <div align="left" class="Estilo5"> Permiso Solicitado:
                                        <% String v_id_todook  = (String)  (((PERMI_data = PERMI.getObject(17))==null || PERMI.wasNull())?"":PERMI_data);%>
                                     </div></td>
                                  </tr>
                                  <tr bgcolor="#f2f2f2">
...
<table width="100%"  border="0" cellspacing="0" cellpadding="4">
 ...
 <tr>
    ...
    <td colspan="2" valign="middle"><span class="Estilo9"><%= (((PERMI_data = PERMI.getObject(16))==null || PERMI.wasNull())?"":PERMI_data)%></span></td>
    </tr>
<tr>
  ...
  <%  } else { %>
    ...
    <td colspan="2"><span class="Estilo11 Estilo6"><%= (((PERMI_data = PERMI.getObject(16))==null || PERMI.wasNull())?"":PERMI_data)%></span></td>
  </tr>
<tr>
    <td colspan="3">
      <div align="center">
        <input type="button" name="button" value="Volver solicitud" onClick="window.location='solicitud_p.jsp?ID_ANO=' + <%= PERMI__V_ID_ANO%>  +  '&V_ID_TIPO_PERMISO=' + '<%= PERMI__V_ID_TIPO_PERMISO%>'
+'&V_ID_TIPO_DIAS=' + '<%= PERMI__V_ID_TIPO_DIAS%>'  
...
+'&V_ID_GRADO=' + '<%= PERMI__V_ID_GRADO%>'  
+'&V_DPROVINCIA=' + '<%= PERMI__V_DPROVINCIA%>'  
+'&V_ID_JUSTIFICACION=' + '<%= PERMI__V_JUSTIFICACION%>'  
+'&V_ID_UNICO=' + '<%= PERMI__V_UNICO%>'
+'' ">
      </div></td>
    </tr>
  <% } %>
