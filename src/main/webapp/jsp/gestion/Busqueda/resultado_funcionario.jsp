...
%>
<%
String RSQUERY_SQL = "SELECT distinct pe.ID_FUNCIONARIO as ID_FUNCIONARIO, INITCAP(APE1) AS APE1, INITCAP(APE2) AS APE2,  INITCAP(NOMBRE) AS NOMBRE,  UPPER(NOMBRE) || ' ' ||UPPER(APE1)|| ' ' ||UPPER(APE2) AS NOMBRES,          DESC_TIPO_FUNCIONARIO,       FECHA_INGRESO,            FECHA_BAJA,       DECODE(nvl(FECHA_BAJA-sysdate,0),'0','0','1') as ESTADO_BAJA   FROM PERSONAL_NEW PE,TR_TIPO_FUNCIONARIO TR  WHERE 1 = 1 and TIPO_FUNCIONARIO2=tr.id_tipo_funcionario and  pe.ID_FUNCIONARIO ||  UPPER(NOMBRE)  ||' ' ||  UPPER(APE1) || ' ' ||    upper(APE2) ||   upper(lpad(dni,8,'0')) || upper(dni_letra)||     UPPER(DESC_TIPO_FUNCIONARIO) like upper('%" + RSQUERY__MMColParam17 + "%') and (tipo_funcionario2= '" + RSQUERY__POLI  + "' OR  '0'= '" + RSQUERY__POLI  + "')";
              ...
              <tr> 
                <td bgcolor="#f2f2f2"> 
                  <table width="100%" border="0" cellspacing="1" cellpadding="2">
                    ...
                    <tr bgcolor="#FFFFFF"> 
                      <td <%= RSCOLOR %> align="center" width="10%">
                        <A HREF='#'   onClick='window.opener.document.getElementById("<%= RSQUERY__Busqueda %>").value=<%=  (((RSQUERY_data = RSQUERY.getObject("ID_FUNCIONARIO"))==null || RSQUERY.wasNull())?"":RSQUERY_data)%>;
                        window.opener.document.getElementById("<%= RSQUERY__Busqueda %>_ID").value=<%=  (((RSQUERY_data = RSQUERY.getObject("ID_FUNCIONARIO"))==null || RSQUERY.wasNull())?"":RSQUERY_data)%>;
                        window.opener.document.getElementById("<%= RSQUERY__Busqueda %>_NOMBRE").value="<%=  (((RSQUERY_data = RSQUERY.getObject("NOMBRES"))==null || RSQUERY.wasNull())?"":RSQUERY_data)%>";
                      
                        window.close()';>  <%= (((RSQUERY_data = RSQUERY.getObject("ID_FUNCIONARIO"))==null || RSQUERY.wasNull())?"":RSQUERY_data) %> </A>
                      </td>
                      ...
                      <td <%= RSCOLOR %>align="center" width="15%"></td>
