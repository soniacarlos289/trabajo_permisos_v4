...
PreparedStatement StatementRSQUERY = ConnRSQUERY.prepareStatement("select distinct p.ID_FUNCIONARIO as ID_FUNCIONARIO,p.Ape1 || ' ' || p.ape2 || ' ' || p.NOMBRE as NOMBREs,pe.fecha_inicio as finc,p.Ape1 , p.ape2 , p.NOMBRE , tr.DESC_tipo_PERMISO as TIPO_PERMISO,  pe.id_permiso as ID_PERMISO,to_char(pe.FECHA_INICIO,'dd/mm/yyyy') as FECHA_INICIO,    to_char(pe.FECHA_FIN,'dd/mm/yyyy') AS FECHA_FIN,pe.NUM_DIAS as NUM_DIAS,   DECODE(a.id_funcionario,NULL,'NO','SI') as INCIDENCIA ,to_char(a.FECHA_AVISO,'dd/mm/yyyy') as FECHA_AVISO from permiso pe,tr_tipo_permiso tr,personal_new p,PERMISOS_AVISOS_SINJUSTI A where pe.id_estado=80 and pe.JUSTIFICACION='NO' and tr.id_tipo_permiso=pe.id_tipo_permiso and    tr.id_ano=pe.id_ano  and     pe.fecha_fin< sysdate and     pe.id_funcionario=p.id_funcionario and     pe.id_funcionario=a.id_funcionario(+) and     pe.id_permiso=a.id_permiso(+) and        pe.fecha_inicio between to_date('01/02/2018', 'dd/mm/yyyy') and sysdate-7 and   (pe.ANULADO='NO' OR pe.anulado is null) and a.valido(+)=1 and  tipo_permiso <> 'Asistencia a cursos y conferencias' order by finc");
...
<ul id="tabh">
  ...
  <div id="form">
    <div>
	  <ul id="subtabh">		
		<li><a href="index.jsp" id="current">Permisos sin Justificar</a></li>
...
<table width="100%" border="1" cellspacing="3" cellpadding="4">
                                      <tr bgcolor="#CCFFCC"> 
                                        <td colspan="7" align="center"><b>Permisos sin Justificar</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;                                           
                                      ...
                                      <tr> 
                                        ...
                                        <td width="20%" bgcolor="#CCCCCC" align="center"><span class="va10b">Fecha Aviso</span></td>
                                        <td width="5%" bgcolor="#CCCCCC" align="center"><span class="va10b">Acciones</span></td>
                                    ...
                                    <td <%= RSCOLOR %> width="20%" align="left" bgcolor="#FFFFFF"><%=(((RSQUERY_data = RSQUERY.getObject("FECHA_AVISO"))==null || RSQUERY.wasNull())?"":RSQUERY_data)%></td>
                                      <td <%= RSCOLOR %> width="20%" align="center" bgcolor="#FFFFFF">  <a href="javascript:show_confirm('<%=(((RSQUERY_data = RSQUERY.getObject("ID_PERMISO")) == null || RSQUERY
							.wasNull()) ? "" : RSQUERY_data)%>','<%=(((RSQUERY_data = RSQUERY.getObject("ID_PERMISO")) == null || RSQUERY
							.wasNull()) ? "" : RSQUERY_data)%>','eliminar.jsp?ID_PERMISO=')"><img	src="../../imagen/eliminar.gif" alt="Eliminar" width="20" height="20" border="0">							
                      </a> </td>
                                      </tr>
                                      <% } /* end !RSQUERY_isEmpty */ %>
