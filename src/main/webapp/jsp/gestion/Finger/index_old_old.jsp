...
PreparedStatement StatementRSPERIODO = ConnRSPERIODO.prepareStatement("select to_char(mod(rownum, 2)) as impar,to_char(f,'dd/mm/yyyy') as f_txt, f, fecha,    to_CHAR(F, 'MM') || '20' || to_CHAR(F, 'YY') AS MES_FECHA,  entrada,     salida,       horas_fichadas,  horas_hacer, fuera_saldo,    round(horas_hacer_m) as horas_hacer_m ,   to_char(round(diferencia_minutos)) as dminutos,    devuelve_min_fto_hora( round(to_char(diferencia_minutos))) as DMINUTOS_F,    observaciones,       mes_fecha_ano,       id_funcionario        from FICHAJE_SALDO_COMPLETA_NEW t where id_funcionario= '" + RSPERIODO__MMColParam1 + "' and mes_fecha_ano =  devuelve_periodo('" + RSPERIODO__MMColParam3 + "')  and   (horas_fichadas_m<>0 OR  horas_hacer_m <>0)  ORDER BY 3,ENTRADA");
...
PreparedStatement StatementRSSALDO = ConnRSSALDO.prepareStatement("SELECT SUM(nvl(a.Campo11,0) + nvl(a.Campo9,0)) AS DM  FROM  webfinger   a,        webperiodo  b,       apliweb_usuario   u  where   lpad(b.mes || b.ano,6,'0') = devuelve_periodo('" + RSSALDO__MMColParam3 +"') and  lpad(u.id_funcionario,6,'0')=lpad('" + RSSALDO__MMColParam1 + "',6,'0')  and a.fecha between b.inicio and b.fin and a.codpers = id_fichaje");
...
PreparedStatement StatementRSFICHA = ConnRSFICHA.prepareStatement("select NVL(PIN,'SIN') as PIN,to_char(nvl(ID_TIPO_FIcHAJE,9))as ID_TIPO_FICHAJE,p.id_funcionario ,   to_char(nvl(p.tipo_funcionario2,0)) as id_tipo_funcionario from FUNCIONARIO_FICHAJE f,personal_new p where f.id_funcionario(+)=p.id_funcionario and p.id_funcionario=" + RSFICHA__MMColParam1 + " ");
					    ...
					    <div align="left">
					      <table border="0" cellspacing="0" cellpadding="2">
					        ...
					        <tr>
					          ...
					          <td class="va12b">&nbsp;<input type="button" value="Configuracion" name="Configuracion" onClick="window.location='index_configuracion.jsp'">&nbsp;<b><%= session.getValue("MM_ID_FUNCIONARIO_NOMBRE") %> <%= session.getValue("MM_ID_FUNCIONARIO_APE1") %> <%= session.getValue("MM_ID_FUNCIONARIO_APE2") %></b>&nbsp;</td>
                                 ...
                                 <% if ( (!RSPERIODO_isEmpty) ) { %>
                                   ...
                                   <% } else { %> 
                                    ...
                                    <% } %>            
                                             ...
                                             </td>  <td><a href='regenera_saldo_periodo.jsp?ID_FUNCIONARIO=<%= RSSALDO_NEW__MMColParam1%>&ID_PERIODO=<%= RSSALDO_NEW__MMColParam2%>' >
                        ...
                        <% if (!RSPERIODO_isEmpty ) { %>
							<table width="90%"  border="0" cellspacing="1" cellpadding="1">
                              <tr>
                                ...
                                <td width="10%" scope="col"> <div align="center"><strong>Fuera Saldo</strong></div></td>
                             ...
                             <tr>                 
                              <tr>
								  ...
								  %>
                                  <td   <%= RSCOLOR %>><div align="center"> 
								  <%=(((RSPERIODO_data = RSPERIODO.getObject("FECHA"))==null || RSPERIODO.wasNull())?"":RSPERIODO_data)%></td>
                                  ...
                                  <td <%= RSCOLOR %>><div align="center"><%=(((RSPERIODO_data = RSPERIODO.getObject("SALIDA"))==null || RSPERIODO.wasNull())?"":RSPERIODO_data)%></div></td>
                                     <td width="10%"<%= RSCOLOR %>><div align="center"><%=(((RSPERIODO_data = RSPERIODO.getObject("FUERA_SALDO"))==null || RSPERIODO.wasNull())?"":RSPERIODO_data)%></div></td>
                                  ...
                                  <% if ( (!fecha_nueva.equals(fecha_old))  &&  (!tipo_funcionario.equals("21")) )   {%>                                  
								   ...
								   <% valor_acumulado = (String) (((RSPERIODO_data = RSPERIODO.getObject("DMINUTOS"))==null || RSPERIODO.wasNull())?"":RSPERIODO_data); 
								      tmp_obs= (String) (((RSPERIODO_data = RSPERIODO.getObject("OBSERVACIONES"))==null || RSPERIODO.wasNull())?"":RSPERIODO_data);
								       if ( fecha_nueva.equals(fecha_old) ) 
								         {
							    	       valor_acumulado =valor_acumulado; 
							             }  
								       if  ( tmp_obs.equals("NO COMPUTA PARA SALDO."))  
								             {
								    	       valor_acumulado ="0"; 
								             } 
								 ...
								 %>
								  ...
								  <% if  (!tipo_funcionario.equals("21"))  {%>
                                    <td <%= RSCOLOR %>><div align="right"><%=   (((RSPERIODO_data = RSPERIODO.getObject("DMINUTOS"))==null || RSPERIODO.wasNull())?"":RSPERIODO_data) %></div></td>
                                   <% } else { %> 
                                     <td <%= RSCOLOR %>><div align="center"></div></td>
