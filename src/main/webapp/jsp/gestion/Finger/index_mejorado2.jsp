...
PreparedStatement StatementRSSALDO_NEW = ConnRSSALDO_NEW.prepareStatement("select devuelve_min_fto_hora(nvl(sum(horas_saldo-horas_hacer),0)) as saldo  from fichaje_funcionario_resu_dia t, omesa.webperiodo ow where id_funcionario = '" + RSSALDO_NEW__MMColParam1 + "'   and mes||ano = devuelve_periodo('" + RSSALDO_NEW__MMColParam2 + "')    and  t.id_dia  between ow.inicio and ow.fin  and t.id_dia<to_date(to_char(sysdate,'dd/mm/yyyy'),'dd/mm/yyyy')");
